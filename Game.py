...
class Game:
    def __init__(self, args, training=True):
        ...
        self.controller_policy = model.PPO(policy, 
                                        self.obs_space,
                                        self.action_space,
                                        self.device, 
                                        self.logger.dir, 
        ...
        self.meta_controller_value_network = model.Critic(self.obs_space, self.action_space)
        self.meta_controller_value_network.critic_network.to(self.device)

        # init buffer
        self.gamma = args.gamma
        self.lam = args.lam
        self.buffer = model.Buffer(self.gamma, self.lam, self.device)
