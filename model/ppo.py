...
class PPO(Base):
    ...
    def __init__(self, 
        ...
        # model
        super().__init__(obs_space, action_space, device, save_path, recurrent)
    ...
    def update_policy(self, buffer):
        ...
        for _ in range(self.epochs):
            for batch in buffer.sample(self.batch_size, self.recurrent):
                ...
                # get policy
                pdf, value, _ = self.model(obs_batch)
                ...
                surr2 = torch.clamp(ratio, 1.0 - self.clip, 1.0 + self.clip) * advantage_batch 
                policy_loss = -torch.min(surr1, surr2).mean()

                value_clipped = values_batch + torch.clamp(value - values_batch, -self.clip, self.clip)
                surr1 = ((value - return_batch)).pow(2)
                surr2 = ((value_clipped - return_batch)).pow(2)
    ...
    def update_policy(self, buffer):
        for _ in range(self.epochs):
            ...
            for batch in buffer.sample(self.batch_size):
                obs_batch, next_obs_batch, _, return_batch, _, _, mask, log_prob_batch, controller_log_prob, meta_controller_prob_batch, meta_controller_value_batch, meta_controller_tensor_batch, done = batch
                # print(f"controller_log_prob: {controller_log_prob.size()}")
                # print(f"meta_controller_value_batch: {meta_controller_tensor_batch.size()}")
                kl_div_per_sample = F.cross_entropy(controller_log_prob, meta_controller_tensor_batch, reduction='mean')
