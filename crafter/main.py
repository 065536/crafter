...
def main():
    ...
    for i in tqdm(range(n_iter), desc="Training Progress", unit="iter"):
        print("\n" * 5)
        print("=" * 50)
        print(f"Start a new game {i + 1}/{n_iter}")
        ...
        with tqdm(total=max_step, desc=f"Game {i + 1} Steps", unit="step") as pbar:
        ...
        if n_games % 10 == 1:
            ...
            if eval_steps:
                ...
                print(f"Game {n_games} finished, Success_Rate: {eval_successes / len(eval_steps)}, Step length: {np.mean(eval_steps)}")
            else:
